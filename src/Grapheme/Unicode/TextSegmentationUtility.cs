...
namespace System.Text.Unicode
{
    ...
    /// </remarks>
    public static class TextSegmentationUtility
    {
        internal delegate OperationStatus DecodeFirstRune<T>(ReadOnlySpan<T> input, out Rune rune, out int elementsConsumed);
        ...
        /// <summary>
        /// Given UTF-8 input text, returns the length (in chars) of the first extended grapheme cluster.
        /// The slice [0..length] represents the first standalone extended grapheme cluster in the text.
        /// If the input is empty, returns 0.
        /// </summary>
        ...
        /// A grapheme cluster is defined as a sequence of codepoints constituting a discrete symbol when displayed.
        /// </summary>
        public static GraphemeClusterEnumerator<byte> GetUtf8ClusterEnumerator(ReadOnlySpan<byte> input) {
            return new(input, _utf8Decoder);
        }
        ...
        /// A grapheme cluster is defined as a sequence of codepoints constituting a discrete symbol when displayed.
        /// </summary>
        public static GraphemeClusterEnumerator<char> GetUtf16ClusterEnumerator(ReadOnlySpan<char> input) {
            return new(input, _utf16Decoder);
        }
        ...
        /// <summary>
        /// Iterates a sequence of codepoints, using a specific unicode decoder, in segments of graphemes.
        /// A grapheme cluster is defined as a sequence of codepoints constituting a discrete symbol when displayed.
        /// </summary>
        public ref struct GraphemeClusterEnumerator<T> {
            private readonly DecodeFirstRune<T> _decoder;
            private readonly ReadOnlySpan<T> _codepoints;
            private ReadOnlySpan<T> _remaining;
            private ReadOnlySpan<T> _currentCluster;

            internal GraphemeClusterEnumerator(ReadOnlySpan<T> codepoints, DecodeFirstRune<T> decoder) {
                _codepoints = codepoints;
                _decoder = decoder;
                _remaining = codepoints;
                _currentCluster = default;
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public bool MoveNext() {
                int clusterWidth = GetLengthOfFirstExtendedGraphemeCluster(_remaining, _decoder);
                _currentCluster = _remaining.Slice(0, clusterWidth);
                _remaining = _remaining.Slice(clusterWidth);
                return clusterWidth == 0;
            }

            public void Reset() {
                this = new(_codepoints, _decoder);
            }

            public readonly ReadOnlySpan<T> Current => _currentCluster;
        }
